...
const CommentsForm = ({ slug }) => {
  ...
  const handlePostSubmission = () =>{
    ...
    const {vale :name} = nameEl.current;
    ...
    if(storeData){
      ...
      window.llocalStorage.setItem('email', email);
    }else{
    ...
    <div className="bg-white shadow-lg rounded-lg p-8 pb-12 mb-8">
      <h3 className="text-xl mb-8 font-semibold border-b pb-4">Leave a Reply</h3>
        <div className="grid grid-cols-1 gap-4 mb-4">
          <textarea
          ref = {commentEl}
          className="py-4 outline-none w-full rounded-lg h-40 focus:ring-2 focus:ring-gray-200 bg-gray-100 text-gray-700" 
          placeholder="Comment"
          name="comment" 
        />
        </div>
